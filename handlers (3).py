...
from database import (
    ...
    add_user_balance,
    deduct_user_balance,
    get_pending_orders_for_admin,  # <-- Bekleyen siparişler için eklendi
)
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin:
        ...
        elif text == "⏳ Bekleyen Siparişler":
            pending_orders = get_pending_orders_for_admin()
            if not pending_orders:
                await update.message.reply_text("🎉 **Harika! Şu anda teslimat bekleyen hiçbir siparişiniz yok.**", parse_mode="Markdown")
                return

            await update.message.reply_text(f"⏳ **Toplam {len(pending_orders)} adet teslimat bekleyen sipariş var:**", parse_mode="Markdown")

            for order in pending_orders:
                order_id, target_telegram_id, username, first_name, product_name, price = order
                user_mention = f"@{username}" if username else first_name
                
                msg_text = (
                    f"🚨 **BEKLEYEN SİPARİŞ #{order_id}**\n\n"
                    f"👤 **Müşteri:** {user_mention}\n"
                    f"🆔 **Telegram ID:** `{target_telegram_id}`\n"
                    f"📦 **Ürün:** {product_name}\n"
                    f"💰 **Ödenen Tutar:** {price} TL\n"
                    f"⏳ **Durum:** Numarası Teslim Edilmedi"
                )
